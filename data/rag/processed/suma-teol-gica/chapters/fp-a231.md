# Primeira Parte (FP)
## Capítulo 231: Whether the Father can be fittingly sent?
...
giving Himself. Therefore it can be said that the Father sends Himself.
...
person; nor even a notion, since there are only five notions, as stated
above ([365]Q[32], A[3]). Therefore every divine person can be sent.
...
Hence, as the Father is not from another, in no way is it fitting for
Him to be sent; but this can only belong to the Son and to the Holy
Ghost, to Whom it belongs to be from another.
...
still He is not described as being sent, for He is not from another.
Thus Augustine says (De Trin. iv, 20) that "The Father, when known by
